...
st.write("スクショからギフト状況を読み取り、選択した箱レベルに応じたパターン別（A・B・C）の最適解を算出します。")
...
if st.session_state.raw_inventory:
    ...
    for mg in MASTER_GIFTS:
        g_info = st.session_state.raw_inventory.get(mg["name"], {"total_count": 0, "nearest_expiry": "なし", "nearest_count": 0})
    ...
    if box_input_raw:
        ...
        if box_input_normalized.isdigit() and len(box_input_normalized) == 1:
            ...
            if box_level in BOX_POPULARITY_MAP:
                ...
                st.write(f"箱レベル {box_level} のキラ値や人気度に到達する最適解を以下のＡ～Ｃのパターンで計算して表示します。")
                
                if st.button("🚀 最適解を計算する"):
                    try:
                        raw_inventory = st.session_state.raw_inventory

                        # --- 現行の最適化ロジック (パターン A, B, C) ---
                        def calculate_plan(mode):
                            ...
                            for item in plan_data:
                                ...
                                table_rows.append({
                                    ...
                                    "人気度": p_val,
                                    "有効期限": item["expiry"],
                                    "有効期限数": item["nearest_count"],
                                    "左記以外の個数": item["other_count"]
                                })
